/**
 * Seed script for production (Turso database).
 * 
 * Usage:
 *   TURSO_DATABASE_URL=libsql://... TURSO_AUTH_TOKEN=... node scripts/seed-production.mjs
 * 
 * Or set the vars in .env and run:
 *   node --env-file=.env scripts/seed-production.mjs
 */
...
if (!url) {
  console.error("❌ TURSO_DATABASE_URL não definida. Use:");
  console.error("   TURSO_DATABASE_URL=libsql://... TURSO_AUTH_TOKEN=... node scripts/seed-production.mjs");
  process.exit(1);
}
...
async function seed() {
  ...
  console.log(`✅ Seed complete: ${inserted} matches inserted, ${skipped} skipped`);
}
